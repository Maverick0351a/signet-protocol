...
DEFAULT_HEADERS = {"User-Agent": "SignetProtocol/0.1", "Content-Type": "application/json"}
    ...
    def send(self, request, *args, **kwargs):
        ...
        try:
            ...
            raise requests.exceptions.ConnectionError(
                f"Failed to connect to {self.target_ip}: {e!s}"
            )


def select_public_ip(hostname: str) -> tuple[bool, str, str | None]:
    """
    ...
    try:
        ...
        infos = socket.getaddrinfo(
            ascii_host, None, family=socket.AF_UNSPEC, type=socket.SOCK_STREAM
        )
        ...
        if parsed.scheme == "https":
            ...
            if not success:
                return {
                    "status_code": 403,
                    "host": hostname,
                    "error": f"IP validation failed: {reason_or_ip}",
                }
