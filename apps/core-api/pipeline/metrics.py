...
repair_success_total = Counter(
    "signet_repair_success_total", "Successful repairs producing valid JSON"
)
fallback_used_total = Counter("signet_fallback_used_total", "Total fallback used")
semantic_violation_total = Counter(
    "signet_semantic_violation_total", "Semantic invariant violations during fallback repair"
)
...
billing_enqueue_total = Counter(
    "signet_billing_enqueue_total", "Billing enqueue operations", ["type"]
)  # type=vex|fu

# Latency histograms
latency_total_hist = Histogram(
    "signet_exchange_total_latency_seconds", "End-to-end exchange latency seconds"
)
phase_latency_hist = Histogram(
    "signet_exchange_phase_latency_seconds", "Per-phase exchange latency seconds", ["phase"]
)

# Gauges for reserved capacity (updated opportunistically; set to last seen values)
reserved_vex_capacity = Gauge(
    "signet_reserved_vex_capacity", "Reserved VEx capacity per tenant", ["tenant"]
)
reserved_fu_capacity = Gauge(
    "signet_reserved_fu_capacity", "Reserved FU capacity per tenant", ["tenant"]
)
