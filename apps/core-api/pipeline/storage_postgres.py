...
class PostgreSQLStorage:
    ...
    def get_chain(self, trace_id: str):
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT * FROM receipts WHERE trace_id = %s ORDER BY hop ASC", (trace_id,)
                )
                rows = cur.fetchall()
    ...
    def dequeue_billing_batch(self, limit: int = 100):
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT * FROM billing_queue ORDER BY id ASC LIMIT %s", (limit,))
                rows = cur.fetchall()
    ...
    def delete_billing_items(self, ids):
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM billing_queue WHERE id = ANY(%s)", (ids,))
            conn.commit()
    ...
    def bump_billing_retries(self, ids):
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE billing_queue SET retries = retries + 1 WHERE id = ANY(%s)", (ids,)
                )
            conn.commit()
