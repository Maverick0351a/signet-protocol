...
class SemanticInvariants:
    ...
    def validate(
        ...
        for rule_name, rule_func in self.rules.items():
            try:
                ...
                print(f"Warning: Invariant rule '{rule_name}' failed: {e!s}")
    ...
    def _check_amount_precision(
        self, original: dict[str, Any], repaired: dict[str, Any], schema: dict[str, Any] | None
    ) -> list[InvariantViolation]:
        """Ensure monetary amounts maintain precision and scale."""
        violations = []

        # Find amount fields
        amount_fields = [k for k in original if "amount" in k.lower()]
    ...
    def _check_currency_unchanged(
        ...
        currency_fields = [k for k in original if "currency" in k.lower() or "curr" in k.lower()]
    ...
    def _check_date_formats(
        ...
        date_fields = [k for k in original if "date" in k.lower() or "time" in k.lower()]
...
def validate_fallback_result(
    original_json: str, repaired_json: str, schema: dict[str, Any] | None = None
