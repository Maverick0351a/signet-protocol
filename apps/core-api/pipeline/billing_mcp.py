...
from .billing import BillingBuffer
from .storage import Storage

# Prometheus metrics
mcp_operations = Counter(
    "signet_mcp_operations_total", "MCP operations performed", ["operation", "status"]
)
stripe_products_created = Counter(
    "signet_stripe_products_created_total", "Stripe products created via MCP"
)
stripe_prices_created = Counter(
    "signet_stripe_prices_created_total", "Stripe prices created via MCP"
)
...
"""
            ...
            raise Exception(f"MCP tool call failed: {e!s}")

    async def create_product(
        self, name: str, description: str, product_type: str = "service"
    ) -> dict[str, Any]:
    ...
    async def create_price(
        self, product_id: str, unit_amount: int, currency: str = "usd", nickname: str | None = None
    ) -> dict[str, Any]:
        """Create a Stripe price using MCP"""
        args = {"product": product_id, "unit_amount": unit_amount, "currency": currency}
        if nickname:
    ...
    async def create_payment_link(
        self, line_items: list[dict[str, Any]], metadata: dict[str, str] | None = None
    ) -> dict[str, Any]:
...
class EnhancedBillingBuffer(BillingBuffer):
    ...
    async def setup_signet_products(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to setup products: {e!s}"}

    async def create_customer_payment_link(
        self, tenant: str, plan_type: str = "monthly"
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to create payment link: {e!s}"}
    ...
    async def get_billing_dashboard_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            for tenant in self.reserved_configs.keys():
                tenant_reports[tenant] = self.generate_monthly_report(tenant, now.year, now.month)
            ...
            return {"success": False, "error": f"Failed to get dashboard data: {e!s}"}
    ...
    async def sync_stripe_items_with_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to sync Stripe items: {e!s}"}


# Factory function to create enhanced billing buffer
def create_enhanced_billing_buffer(
    storage: Storage, stripe_api_key: str | None, reserved_config_path: str | None = None
) -> EnhancedBillingBuffer:
