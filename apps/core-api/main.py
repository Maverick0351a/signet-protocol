...
from .settings import create_storage_from_settings, load_settings
from .utils.crypto import load_signing_key, make_jwk_from_signing_key, sign_export_bundle
from .utils.jcs import cid_for_json
...
def reload_reserved(x_signet_api_key: str | None = Header(None, alias="X-SIGNET-API-Key")):
...
async def setup_stripe_products(
    x_signet_api_key: str | None = Header(None, alias="X-SIGNET-API-Key"),
):
...
async def sync_stripe_items(x_signet_api_key: str | None = Header(None, alias="X-SIGNET-API-Key")):
...
def exchange(
    req: Request,
    body: dict[str, Any],
    x_odin_api_key: str | None = Header(None, alias="X-ODIN-API-Key"),
    x_signet_api_key: str | None = Header(None, alias="X-SIGNET-API-Key"),
    x_odin_idempotency_key: str | None = Header(None, alias="X-ODIN-Idempotency-Key"),
    x_signet_idempotency_key: str | None = Header(None, alias="X-SIGNET-Idempotency-Key"),
):
    t0 = time.time()
    ...
    try:
        ...
        if args_obj is None and tenant_cfg.fallback_enabled:
            try:
                ...
                quota_allowed, quota_reason = fb.check_tenant_fu_quota(
                    tenant_cfg.dict(), estimated_tokens
                )

                if not quota_allowed:
                    raise HTTPException(
                        status_code=429, detail=f"Fallback quota exceeded: {quota_reason}"
                    )
                ...
                if repair_result.success and repair_result.repaired_text:
                    ...
                    is_valid, error_messages, violations = validate_fallback_result(
                        args_str, repair_result.repaired_text, {"type": "object"}
                    )
        ...
        raise HTTPException(
            status_code=422, detail=f"arguments parse/repair failed: {str(e)[:200]}"
        )
    ...
    with phase("validate_output"):
        try:
            ...
            raise HTTPException(
                status_code=422, detail=f"normalized schema invalid: {str(e)[:200]}"
            )
    ...
    receipt = make_receipt(
        trace_id,
        hop=(head["last_hop"] + 1 if head else 1),
    ...
    with phase("record_usage"):
        STORE.record_usage(
            api_key, tenant_cfg.tenant, trace_id, hop, True, 1, fu_tokens_used, receipt["ts"]
        )
        vex_units_total.inc()
        if fu_tokens_used:
    ...
    if fu_tokens_used > 0 and tenant_cfg.stripe_item_fu:
        with phase("billing_enqueue_fu"):
            BB.enqueue_fu(
                api_key, tenant_cfg.stripe_item_fu, fu_tokens_used, tenant=tenant_cfg.tenant
            )
